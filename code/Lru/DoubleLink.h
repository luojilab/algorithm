...
namespace domain_algorithm_share {
	...
	public:
		...
		DNode<T> *get_first();//获取首节点
		DNode<T> *get_last();//获取尾节点
	...
	// 获取第1个节点的值
	template<class T>
	DNode<T> *DoubleLink<T>::get_first()
	{
		return get_node(0);
	}
	...
	// 删除index位置的节点
	template<class T>
	int DoubleLink<T>::delete_index(int index)
	{
		DNode<T>* pindex = get_node(index);
	...
	// 删除node节点
	template<class T>
	int DoubleLink<T>::delete_node(DNode<T> *node)
	{
